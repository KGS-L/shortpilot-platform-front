...
export function Sidebar({space,mobile=false}: {space:Space;mobile?:boolean}) {
  ...
  return <aside className={cn("h-screen w-64 shrink-0 overflow-y-auto border-r bg-white p-5",mobile?"flex flex-col":"sticky top-0 hidden lg:flex lg:flex-col")}><Link href="/" className="mb-8 block text-xl font-black tracking-tight">Omnelyo</Link><p className="mb-3 px-3 text-[11px] font-black uppercase tracking-[.15em] text-slate-400">{label}</p><nav className="space-y-1">{items.map(([name,href,Icon])=>{const active=pathname===href;return <Link key={href} href={href} className={cn("flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition",active?"bg-[#172033] text-white":"text-slate-500 hover:bg-slate-100 hover:text-slate-900")}><Icon size={18}/>{name}</Link>;})}</nav><div className="mt-auto"><Link href="/" className="flex items-center gap-3 px-3 py-2 text-sm font-semibold text-slate-500"><LogOut size={17}/> Quitter l’espace</Link></div></aside>;
}
