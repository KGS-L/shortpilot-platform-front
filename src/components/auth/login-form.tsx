...
export function LoginForm({nextPath="/dashboard"}:{nextPath?:string}) {
  ...
  function complete(tokens:Tokens) { localStorage.setItem("shortpilot_access_token",tokens.access_token); localStorage.setItem("shortpilot_refresh_token",tokens.refresh_token); router.push(nextPath.startsWith("/")&&!nextPath.startsWith("//")?nextPath:"/dashboard"); }
  ...
  return <><Script src="https://accounts.google.com/gsi/client" strategy="afterInteractive" onLoad={initGoogle}/><div className="w-full max-w-md"><p className="text-sm font-black uppercase tracking-[.16em] text-lime-700">Bienvenue à bord</p><h1 className="mt-3 text-4xl font-black tracking-[-.045em]">{step==="email"?"Créez. Publiez. Avancez.":"Consultez votre e-mail."}</h1><p className="mt-3 leading-7 text-slate-600">{step==="email"?"Connectez-vous ou créez votre compte en quelques secondes.":`Nous avons envoyé un code sécurisé à ${email}.`}</p>
    ...
    {step==="code"&&<button onClick={()=>{setStep("email");setCode("");setError("");}} className="mt-5 text-sm font-bold underline">Utiliser une autre adresse</button>}<p className="mt-7 text-xs leading-5 text-slate-500">En continuant, vous acceptez les conditions d’utilisation et la politique de confidentialité de ShortPilot.</p></div></>;
}
